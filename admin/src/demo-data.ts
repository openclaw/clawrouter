...
export function demoData() {
  ...
  const keys: AccessPolicy[] = [
    { policyId: "maintainer_models", enabled: true, providers: ["anthropic", "aws-bedrock", "azure-openai", "cloudflare-ai-gateway", "cohere", "deepseek", "fireworks", "google-gemini", "groq", "huggingface", "minimax", "mistral", "openai", "openrouter", "perplexity", "together", "xai"], tenantId: "openclaw", tokenRole: "maintainer", monthlyBudgetMicros: 250000000, requestCostMicros: 1000, retainRequestContent: true, grantRouting },
    ...
    { policyId: "user_research", enabled: true, providers: ["openai", "google-gemini", "tavily"], tenantId: "research", tokenRole: "user", monthlyBudgetMicros: 50000000, requestCostMicros: 1000, retainRequestContent: true, grantRouting },
  ...
  const upstreamGrants: UpstreamGrant[] = [
    { key: "oauth/maintainer_models/openai", scope: "policies", scopeId: "maintainer_models", tokenRef: "openai", version: 1, enabled: true, priority: 10, weight: 2, kind: "subscription", provider: "openai", label: "maintainer subscription", tokenType: "Bearer", scopes: ["openid", "profile"], accountId: "acct_demo", subscription: { plan: "plus" }, createdAt: "2026-06-16T00:00:00.000Z", updatedAt: "2026-06-16T00:00:00.000Z", hasCredential: false, credentialFields: [], hasAccessToken: true, hasRefreshToken: true, refreshConfigured: true, usable: true, selectedCount: 84, lastSelectedAt: "2026-06-16T11:59:00.000Z", quotaStatus: "available", quotaObservedAt: "2026-06-16T11:58:00.000Z", cooldownUntil: null, quotaSource: "provider_response", lastProviderSignal: "quota", quotaWindows: [{ id: "rpm", kind: "requests", unit: "request", window: "minute", remaining: 812, limit: 1_000, resetAt: "2026-06-16T12:00:00.000Z" }] },
    { key: "oauth/tenants/openclaw/anthropic", scope: "tenants", scopeId: "openclaw", tokenRef: "anthropic", version: 1, enabled: true, priority: 100, weight: 1, kind: "api_key", provider: "anthropic", label: "shared Anthropic key", tokenType: "Bearer", scopes: [], createdAt: "2026-06-16T00:00:00.000Z", updatedAt: "2026-06-16T00:00:00.000Z", hasCredential: true, credentialFields: [], hasAccessToken: false, hasRefreshToken: false, refreshConfigured: false, usable: true, selectedCount: 12, lastSelectedAt: null, quotaStatus: "unknown", quotaObservedAt: null, cooldownUntil: null, quotaSource: null, lastProviderSignal: null, quotaWindows: [] },
  ];
