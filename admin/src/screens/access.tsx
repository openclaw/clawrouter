...
export function PoliciesScreen({ upstream, grantPoolRecovery, tab, setTab, keys, selected, credentials, selectedCredential, bindings, selectedBinding, assignmentRules, selectedAssignmentRule, fusionConfig, fusionReadiness, fusionPolicyId, onSelectFusionPolicy, setFusionConfig, fusionModels, providers, form, setForm, credentialForm, setCredentialForm, bindingForm, setBindingForm, assignmentRuleForm, setAssignmentRuleForm, credentialFeedback, error, policyError, policyDirty, policyMissing, policyReady, policyBusy, onDiscardPolicy, fusionError, onSave, onIssueCredential, onRevokeCredential, onRotateCredential, onNewCredential, onSaveBinding, onSaveAssignmentRule, onReconcileAssignments, onSaveFusion, onCheckFusion, onNew, onEdit, onEditCredential, onEditBinding, onNewBinding, onEditAssignmentRule, onNewAssignmentRule, onRevoke, onPreset, onToggleProvider, onSetProviderGroup, busy }: {
  ...
  onRevokeCredential: (credential: ProxyCredential) => void;
  ...
  return (
    <div className="entityLayout">
      <section className="mainPane">
        ...
        {model.creations.map(entry => <section key={entry.key} className="formGrid compact" aria-label={`Creation recovery ${entry.tokenRef}`}>
          <div className="full"><strong>{entry.creation?.status === "pending" ? "Account creation in progress" : "Account creation unconfirmed"}</strong><p>This request keeps its own reference when you open another draft. Checking reads current facts; it cannot confirm the original request or replay it.</p></div>
          <dl className="facts full"><dt>account reference</dt><dd>{entry.tokenRef}</dd><dt>requested account</dt><dd>{entry.creation?.requested.label || "No label"} · {entry.creation?.requested.provider}</dd><dt>scope</dt><dd>{entry.scope} · {entry.scopeId}</dd><dt>owner observation</dt><dd>{entry.source === "owner" ? `version ${entry.credentialGeneration} · publication ${entry.publication}` : "not checked"}</dd></dl>
          {entry.creation?.error ? <div className="full"><InlineError message={entry.creation.error} /></div> : null}
          <div className="inspectorActions full"><button type="button" className="buttonSecondary accountRecoveryCheck" aria-disabled={busy || entry.creation?.status === "pending"} onClick={() => void model.checkCreation(entry)}>{entry.creation?.inspection === "loading" ? "Checking account…" : "Check this account"}</button><button type="button" disabled={busy || entry.source !== "owner" || entry.creation?.inspection !== "ready"} onClick={() => model.reviewCreation(entry)}>Review this account</button></div>
          <div className="full"><small>Review opens only this account's saved values with empty secret fields. Your next explicit decision resolves the draft; it does not confirm the old request. For incomplete publication, use <a href="#account-publication-recovery">Repair account publication</a>, then check again.</small></div>
        </section>)}
      ...
      <aside className="inspector">
        <form onSubmit={model.save}>
          ...
          {error ? <InlineError message={error} /> : null}
          {facts?.source === "owner" && facts.publication === "pending" ? <InlineNote><strong>Account publication pending.</strong> The owner reports incomplete routing or reporting publication. Use <a href="#account-publication-recovery">Repair account publication</a>, then check this account again. Checking alone does not repair it or acknowledge a lost write.</InlineNote> : null}
...
function quotaWindowLabel(window: UpstreamGrant["quotaWindows"][number]): string {
  ...
  return `${window.id}${window.window ? ` (${window.window})` : ""} ${remaining}/${limit}${window.unit ? ` ${window.unit}` : ""}${window.resetAt ? ` resets ${quotaTimestamp(window.resetAt)}` : ""}`;
}
...
export function CredentialPanel({ policies, credentials, selected, form, setForm, feedback, onIssue, onEdit, onRevoke, onRotate, onNew, busy }: {
  policies: AccessPolicy[];
  credentials: ProxyCredential[];
  ...
  onRevoke: (credential: ProxyCredential) => void;
  onRotate: (credential: ProxyCredential) => void;
  onNew: () => void;
  busy: boolean;
}) {
  ...
  const policyAvailable = policies.some((policy) => policy.policyId === form.policyId && policy.enabled);
  return (
    <div className="entityLayout">
      <section className="mainPane">
        <div className="overviewStrip">
        ...
        <div className="tableSectionHeader"><div><strong>Issued credentials</strong><span>Machine access bound to policies</span></div><button type="button" onClick={onNew}><Plus className="buttonIcon" aria-hidden="true" /><span>New credential</span></button></div>
      ...
      <aside className="inspector">
        <InspectorHeader icon={KeyRound} title={selected ? "Selected credential" : "New credential"} subtitle={selected ? "rotate its secret or revoke access" : "creates a new secret for one policy"} />
        <CredentialNotice state={feedback} />
        {selected ? <>
          <dl className="facts"><dt>id</dt><dd>{selected.credentialId}</dd><dt>policy</dt><dd>{selected.policyId}</dd><dt>owner</dt><dd>{selected.principalId ?? "unassigned"}</dd><dt>state</dt><dd>{selectedOutcome?.label ?? "inactive"}</dd></dl>
          <InlineNote>Rotation replaces only the secret. The policy and owner stay the same; the previous secret stops working.</InlineNote>
          <div className="inspectorActions"><button type="button" className="buttonSecondary" disabled={busy || !selected.active} onClick={() => onRotate(selected)}>Rotate credential</button><button type="button" className="buttonDanger" disabled={busy || !selected.enabled} onClick={() => onRevoke(selected)}><CircleSlash2 className="buttonIcon" aria-hidden="true" /><span>Revoke credential</span></button></div>
        </> : <form onSubmit={onIssue}>
          <div className="formGrid compact">
            <label className="full"><span>credential id</span><input value={form.credentialId} maxLength={128} onChange={(event) => setForm({ ...form, credentialId: event.target.value })} placeholder="auto-generated when blank" /></label>
            <label className="full"><span>policy</span><select value={form.policyId} onChange={(event) => setForm({ ...form, policyId: event.target.value })}>{!policies.some((policy) => policy.policyId === form.policyId) ? <option value={form.policyId}>{form.policyId ? `${form.policyId} · unavailable` : "Select a policy"}</option> : null}{policies.map((policy) => <option key={policy.policyId} value={policy.policyId} disabled={!policy.enabled}>{policy.policyId}{policy.enabled ? "" : " · disabled"}</option>)}</select></label>
          ...
          {form.policyId && !policyAvailable ? <InlineNote>This policy is unavailable. Choose an enabled policy before issuing the credential.</InlineNote> : null}
          ...
          <div className="inspectorActions"><button type="submit" disabled={busy || !policyAvailable}><Plus className="buttonIcon" aria-hidden="true" /><span>Issue credential</span></button></div>
        </form>}
      </aside>
    </div>
  );
