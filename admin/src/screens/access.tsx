      ...
      <aside className="inspector">
        <form onSubmit={onSave}>
          ...
          <div className="formGrid compact">
            ...
            {form.kind === "subscription" ? <label className="full"><span>keep warm</span><select value={form.keepWarm ? "enabled" : "disabled"} onChange={(event) => setForm({ ...form, keepWarm: event.target.value === "enabled" })}><option value="disabled">disabled · quota polling only</option><option value="enabled">enabled · periodic minimal request</option></select></label> : null}
          ...
          <InlineNote>Lower priorities are fallback tiers. The policy chooses how grants within the active tier rotate. Weight affects weighted and sticky selection. Subscription quota polling is automatic; keep-warm is opt-in because it consumes provider capacity. Secret values are write-only.</InlineNote>
