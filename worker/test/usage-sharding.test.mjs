...
import { mergeUsageSnapshots, usageCutoffs, usageDayMs, usageShardName } from "../usage-sharding.ts";
...
test("usage charts use UTC buckets without changing rolling summary semantics", () => {
  const now = Date.UTC(2026, 6, 6, 18, 30);
  const cutoffs = usageCutoffs(now, 30);
  assert.equal(cutoffs.rolling, now - 30 * usageDayMs);
  assert.equal(cutoffs.daily, Date.UTC(2026, 5, 7));
});
